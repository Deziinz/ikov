    ...
    public static Tile validTile(){
    	...
    	for(int i=0; i < 4; i++){
    		for(SceneObject obj : SceneObjects.getNearest()){
    			if(obj.getLocation().getX() == X-i && obj.getLocation().getY() == Y){
    				break;
    			} else if(new Tile(X-i,Y).isReachable()){
    	    		System.out.println("Found valid tile: "+(X-i) +", "+Y);
    	    		X -= i;
    	    		return new Tile(X, Y);
    			}
        	}
    		for(SceneObject obj : SceneObjects.getNearest()){
    			if(obj.getLocation().getX() == X && obj.getLocation().getY() == Y+i){
    				break;
    			} else if(new Tile(X,(Y+i)).isReachable()){
    	    		System.out.println("Found valid tile: "+X +", "+(Y+i));
    	    		Y += i;
    	    		return new Tile(X, Y);
    			}
        	}
