		...
		if(Game.getOpenBackDialogId() == 2469){
			...
			if(Npcs.getNearest(PDungeoneering.THOK_ID).length > 0){
				if(Npcs.getNearest(PDungeoneering.THOK_ID) != null){
					if(Npcs.getNearest(PDungeoneering.THOK_ID)[0] != null){
						Menu.sendAction(225, Npcs.getNearest(PDungeoneering.THOK_ID)[0].getIndex(), 0, 0, 0, 4);//open start dung interface
						Time.sleep(new SleepCondition(){
							public boolean isValid() {
